...
theorem testThm (l : List Nat) : Nonempty (deduped_list l) :=
  let deduped_nil : deduped_list [] := ⟨ [], nodup_nil, subset_nil [], subset_nil [] ⟩
  let deduped_cons (hd : Nat) (tl : List Nat) (prev: deduped_list tl) : deduped_list (hd :: tl) :=
  ...
  let dedup_list : ∀ (l : List Nat), deduped_list l :=
    λ l => List.recOn l deduped_nil (λ hd tl prev => deduped_cons hd tl prev)
  Nonempty.intro (dedup_list l)
