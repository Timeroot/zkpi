...
theorem sub_eff_is_zero_succ {n m: Nat} : (is_zero (sub_eff n m)) → (is_zero (sub_eff n m.succ)) :=
  λ h =>
    have h₂ : (fun a => is_zero (Nat.rec 0 (fun pprev _x => pprev) a)) (sub_eff n m) := @Eq.subst Nat
...
theorem sub_eff_is_zero_pred {n m: Nat} : (¬ is_zero (sub_eff n m.succ)) → (¬ is_zero (sub_eff n m)) :=
...
theorem testThm (l : Nat_list) : (lt_eff (list_len l) (sum_list l)) → (double_pigeon l) :=
