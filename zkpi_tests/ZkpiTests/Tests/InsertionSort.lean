  ...
  @List.recOn Nat (fun l => (∀ (h: sorted l), sorted (insert_sorted x l))) l
    ...
    (λ hd tl prev =>
      (λ h: sorted (hd :: tl) => Decidable.recOn (decidable_bool_eq (le_eff x hd))
        (λ h_1 => by
          have h_2 := @Eq.subst Bool (λb => sorted (Bool.rec (hd :: List.rec [x] (λ (hd : Nat) (tl prev : List Nat) => Bool.rec (hd :: prev) (x :: hd :: tl) (le_eff x hd)) tl) (x :: hd :: tl) b)) Bool.true (le_eff x hd) (flip_eq (not_eq_ff_eq_tt h_1)) (sorted_cons_le x hd tl h (not_eq_ff_eq_tt h_1))
          exact h_2)
                    ...
                    (λ pp =>
                        @Eq.subst Bool (λ b1 => sorted (hd :: Bool.rec (l_tl_hd :: List.rec [x] (λ (hd : Nat) (tl prev : List Nat) => Bool.rec (hd :: prev) (x :: hd :: tl) (le_eff x hd)) l_tl_tl) (x :: l_tl_hd :: l_tl_tl) b1)) Bool.false (le_eff x l_tl_hd) (flip_eq pp)
                          (
                            ...
                            let h4 := l_ih h3
                            let h5 := And.left h
                            let h6 := @Eq.subst Bool (λ b1 => sorted (Bool.rec (l_tl_hd :: List.rec [x] (λ (hd : Nat) (tl prev : List Nat) => Bool.rec (hd :: prev) (x :: hd :: tl) (le_eff x hd)) l_tl_tl) (x :: l_tl_hd :: l_tl_tl) b1)) (le_eff x l_tl_hd) Bool.false (pp) (by exact h4)
